...
export class CanonicalDocumentEvidenceService {
  ...
  async retainExtraction(input: { userId: string; practiceId: string; clientEntityId: string; ledgerBookId: string; bytes: Buffer; storageLocator: string; filename: string | null; mimeType: string; extraction: InvoiceExtraction; extractorName: string; extractorVersion: string }): Promise<EvidenceIngestResult> {
    ...
    const { data, error } = await this.store.rpc("ingest_document_evidence_v1", {
      p_practice_id: input.practiceId, p_client_entity_id: input.clientEntityId, p_ledger_book_id: input.ledgerBookId,
      p_actor_user_id: input.userId, p_content_sha256_hex: contentSha256, p_content_length: input.bytes.length,
  ...
  async confirm(input: { userId: string; practiceId: string; clientEntityId: string; ledgerBookId: string; extractionId: string; idempotencyKey: string; documentKind: string; confirmedRevision: Record<string, unknown> }): Promise<EvidenceConfirmationResult> {
    const fingerprint = sha256Hex(canonicalJson({ documentKind: input.documentKind, confirmedRevision: input.confirmedRevision }));
    const { data, error } = await this.store.rpc("confirm_document_evidence_v1", {
      p_practice_id: input.practiceId, p_client_entity_id: input.clientEntityId, p_ledger_book_id: input.ledgerBookId,
      p_actor_user_id: input.userId, p_extraction_id: input.extractionId, p_idempotency_key: input.idempotencyKey,

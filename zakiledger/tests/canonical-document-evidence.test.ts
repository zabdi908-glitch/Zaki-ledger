...
const base = { userId: "user-a", practiceId: "practice-a", clientEntityId: "client-a", ledgerBookId: "book-a" };
...
describe("Eyes to Memory canonical evidence bridge", () => {
  it("hashes immutable bytes and extraction deterministically for duplicate upload replay", async () => {
    const calls: any[] = [];
    const service = new CanonicalDocumentEvidenceService({ rpc: async (_n, v) => { calls.push(v); return { data: { outcome: "CREATED", artifactId: "artifact", extractionId: "extract" }, error: null }; } });
    ...
    await service.retainExtraction({ ...base, bytes: Buffer.from("same source"), storageLocator: "client-a/a", filename: "a.pdf", mimeType: "application/pdf", extraction, extractorName: "test", extractorVersion: "1" });
    expect(calls[0].p_content_sha256_hex).toBe(calls[1].p_content_sha256_hex);
    expect(calls[0].p_extraction_fingerprint_hex).toBe(calls[1].p_extraction_fingerprint_hex);
  });

  it("binds confirmation idempotency to all confirmed facts and exact canonical scope", async () => {
    const calls: any[] = [];
    const service = new CanonicalDocumentEvidenceService({ rpc: async (_n, v) => { calls.push(v); return { data: { outcome: "RESUMED", documentId: "doc", revisionId: "rev" }, error: null }; } });
    ...
    expect(calls[0].p_confirmed_fingerprint_hex).toBe(calls[1].p_confirmed_fingerprint_hex);
    expect(calls[0].p_confirmed_fingerprint_hex).not.toBe(calls[2].p_confirmed_fingerprint_hex);
    expect(calls[0]).toMatchObject({ p_client_entity_id: "client-a", p_ledger_book_id: "book-a", p_actor_user_id: "user-a" });
  });
